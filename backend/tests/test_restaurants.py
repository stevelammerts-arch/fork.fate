...
class TestRestaurantsAPI:
    def test_get_restaurants_seeded(self, client):
        ...
        # curated pool has grown across iterations — assert invariants
        assert len(data) >= 31
    ...
    def test_create_and_delete_restaurant(self, client):
        ...
        # Community submissions now await admin review (moderation model)
        assert created["status"] == "pending"
        rid = created["id"]

        # Pending submissions are hidden from the public list
        g = client.get(f"{API}/restaurants")
        assert not any(x["id"] == rid for x in g.json())

        # ...but visible to the admin, who can reject (delete) them
        auth = {"Authorization": f"Bearer {mint_admin_token()}"}
        subs = client.get(f"{API}/admin/submissions", headers=auth)
        assert subs.status_code == 200
        assert any(x["id"] == rid for x in subs.json())

        d = client.delete(f"{API}/admin/submissions/{rid}", headers=auth)
        assert d.status_code == 200

        subs2 = client.get(f"{API}/admin/submissions", headers=auth)
        assert not any(x["id"] == rid for x in subs2.json())

    def test_delete_nonexistent_404(self, client):
        auth = {"Authorization": f"Bearer {mint_admin_token()}"}
        r = client.delete(f"{API}/admin/submissions/nonexistent-uuid-xyz", headers=auth)
        assert r.status_code == 404
