...
import { REAPER_SRC, loadImage, buildBadge } from "../lib/crawlBadge";
...
/** Live preview of the dark/reaper reward card (mirrors buildReaperBadge geometry). */
function DarkBadgePreview({ photo, name, crew, label, t }) {
  return (
...
/** Live preview of the light/seasonal horizontal certificate card. */
function LightBadgePreview({ photo, name, crew, label, ac }) {
  return (
    <div
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className={`max-h-[94vh] overflow-y-auto sm:max-w-xl ${dlg}`} data-testid="crawl-badge-dialog">
        ...
        {!light ? (
          <DarkBadgePreview photo={photo} name={name} crew={crew} label={label} t={t} />
        ) : (
          <LightBadgePreview photo={photo} name={name} crew={crew} label={label} ac={ac} />
        )}
